...
public class MipsXray extends AbstractMarsToolAndApplication {
	...
	private DatapathAnimation datapathAnimation; // class panel that runs
													// datapath animation.
	...
	protected JComponent buildMainDisplayArea(String figure) {
		...
		try {
			...
			System.out
					.println("Load Image error for " + getClass().getResource(Globals.imagesPath + figure) + ":\n" + e);
			e.printStackTrace();
		}
		System.setProperty("sun.java2d.translaccel", "true");
	...
	// Function that gets the current instruction in memory and start animation
	...
	class Vertex {
		...
		public Vertex(int index, int init, int end, String name, int oppositeAxis, boolean isMovingXaxis,
			...
			for (int i = 0; i < list.length; i++) {
				...
				// Integer.parseInt(list[i])+ " in target");
			}
	...
	// Internal class that set the parameters value, control the basic behavior
	// of the animation , and execute the animation of the
	...
	class DatapathAnimation extends JPanel implements ActionListener, MouseListener {
		...
		public void mousePressed(MouseEvent e) {
			...
			// System.out.println("olha, capturado x=" + a.getLocation().getX()
		...
		// set the binnary opcode value of the basic instructions of MIPS
		// instruction set
		...
		public void importXmlStringData(String xmlName, HashMap table, String elementTree, String tagId,
				String tagData) {
		...
		public void importXmlDatapathMap(String xmlName, String elementTree) {
			...
			try {
				...
				NodeList index_vertex, name, init, end, color, other_axis, isMovingXaxis, targetVertex, sourceVertex,
						isText;
				NodeList datapath_mapList = root.getElementsByTagName(elementTree);
				for (int i = 0; i < datapath_mapList.getLength(); i++) { // extract
																			// the
																			// vertex
																			// of
																			// the
					...
					if (instructionCode.substring(0, 6).equals("000000")) {// R-type
																			// instructions
						color = datapath_mapItem.getElementsByTagName("color_Rtype");
						// System.out.println("rtype");
					} else if (instructionCode.substring(0, 6).matches("00001[0-1]")) { // J-type
																						// instructions
						color = datapath_mapItem.getElementsByTagName("color_Jtype");
						// System.out.println("jtype");
					} else if (instructionCode.substring(0, 6).matches("100[0-1][0-1][0-1]")) { // LOAD
																								// type
																								// instructions
						color = datapath_mapItem.getElementsByTagName("color_LOADtype");
						// System.out.println("load type");
					} else if (instructionCode.substring(0, 6).matches("101[0-1][0-1][0-1]")) { // LOAD
																								// type
																								// instructions
						color = datapath_mapItem.getElementsByTagName("color_STOREtype");
						// System.out.println("store type");
					} else if (instructionCode.substring(0, 6).matches("0001[0-1][0-1]")) { // BRANCH
																							// type
																							// instructions
		...
		// Set up the information showed in the screen of the current
		// instruction.
		public void setUpInstructionInfo(Graphics2D g2d) {
			...
			if (instructionCode.substring(0, 6).equals("000000")) { // R-type
			...
			else if (instructionCode.substring(0, 6).matches("00001[0-1]")) { // jump
																				// intructions
				textVariable = new TextLayout("JUMP TYPE INSTRUCTION", new Font("Verdana", Font.BOLD, 25), frc); // description
																													// of
																													// instruction
																													// code
																													// type
				...
				textVariable.draw(g2d, 105, 500);
																						...
																						// instruction
				textVariable = new TextLayout("LOAD TYPE INSTRUCTION", new Font("Verdana", Font.BOLD, 25), frc); // description
																													// of
																													// instruction
																													// code
																													// type
			...
			else if (instructionCode.substring(0, 6).matches("101[0-1][0-1][0-1]")) {// store
																						// instruction
		...
		// public boolean printTrackRtoL(int init, int end ,int currentIndex,
		// Graphics2D g2d, Color color, int otherAxis,
		...
		// public boolean printTrackDtoU(int init, int end ,int currentIndex,
		// Graphics2D g2d, Color color, int otherAxis,
		// boolean active, boolean firstInteraction){
		...
		// public boolean printTrackUtoD(int init, int end ,int currentIndex,
		// Graphics2D g2d, Color color, int otherAxis,
		// boolean active, boolean firstInteraction){
		...
		public void printTextDtoU(Vertex v) {
			...
			if (instructionCode.substring(0, 6).matches("101[0-1][0-1][0-1]")
					...
					&& !instructionCode.substring(0, 6).matches("00001[0-1]")) {// load
																				// instruction
			...
			if (instructionCode.substring(0, 6).matches("0001[0-1][0-1]") && v.getName().equals("CP+4")) // branch
																											// code
		...
		// set and execute the information about the current position of each
		// line of information in the animation,
		// verifies the previous status of the animation and increment the
		// position of each line that interconnect the unit function.
		...
		public void mouseClicked(MouseEvent e) {
			...
			PointerInfo a = MouseInfo.getPointerInfo();
			// limpar a imagem do painel e iniciar o detalhe da unidade
			// funcional.
