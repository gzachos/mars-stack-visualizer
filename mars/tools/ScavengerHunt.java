...
public class ScavengerHunt implements Observer, MarsTool {
	...
	private static final int NUM_PLAYERS = 22; // Number of players in the game,
												// including "baseline" player
	...
	private static final double MAX_MOVE_DISTANCE = 2.5; // Max. distance
															// (Euclidean
															// measure)
	private static final int ENERGY_AWARD = 20; // Energy awarded for each task
												// completion
	private static final int ENERGY_PER_MOVE = 1; // Energy used in making each
													// move (regardless of
													// distance)
	private static final int SIZE_OF_TASK = 20; // Number of elements in the
												// task

	private static final int NUM_LOCATIONS = 7; // Number of locations to which
												// ScavengerHunt players travel.
	// The first (n-1) locations are "random" locations, the last is
	// START_AND_END_LOCATION

	private static final int START_AND_END_LOCATION = 255; // Start and end
															// location of the
															// ScavengerHunt
	private static final int ADMINISTRATOR_ID = 999; // Special ID for
														// administrator

	// MIPS addresses of administrative memory space.
	// The administrator MIPS program writes a value to the Authentication field
	// prior to writing a new
	// value to the PlayerID field. The value of the Authentication field is not
	// itself verified until
	// the PlayerID field changes. Each new value of the Authentication field is
	// one of some sequence
	// known to the Tool program (one-time pad system).
	// Each change of data in the PlayerID field is a signal for the Tool to
	// check the value of the Authentication
	// field. If the value of the Authentication field is correct, the value of
	// the PlayerID field is used
	// for memory bounds checking. A new value of the Authentication field is
	// expected at each change of the
	// PlayerID field (one-time pad system).
	private static final int ADDR_AUTHENTICATION = 0xffffe000; // MIPS byte
																// address of
																// Authentication
																// field
	private static final int ADDR_PLAYER_ID = 0xffffe004; // MIPS byte address
															// of PlayerID field
	private static final int ADDR_GAME_ON = 0xffffe008; // MIPS byte address of
														// signal that
														// administration has
														// initialized data
	private static final int ADDR_NUM_TURNS = 0xffffe00c; // MIPS byte address
															// of number of
															// turns remaining
															// in the game
	...
	private static final int ADDR_BASE = 0xffff8000; // MIPS byte address of
														// memory space for
														// first player
	private static final int MEM_PER_PLAYER = 0x400; // MIPS bytes of memory
														// space given to each
														// player
	private static final int OFFSET_WHERE_AM_I_X = 0x0; // MIPS byte offset to
														// this field
	private static final int OFFSET_WHERE_AM_I_Y = 0x4; // MIPS byte offset to
														// this field
	private static final int OFFSET_MOVE_TO_X = 0x8; // MIPS byte offset to this
														// field
	private static final int OFFSET_MOVE_TO_Y = 0xc; // MIPS byte offset to this
														// field
	private static final int OFFSET_MOVE_READY = 0x10; // MIPS byte offset to
														// this field
	private static final int OFFSET_ENERGY = 0x14; // MIPS byte offset to this
													// field
	private static final int OFFSET_NUMBER_LOCATIONS = 0x18; // MIPS byte offset
																// to this field
	private static final int OFFSET_PLAYER_COLOR = 0x1c; // MIPS byte offset to
															// this field
	private static final int OFFSET_SIZE_OF_TASK = 0x20; // MIPS byte offset to
															// this field
	private static final int OFFSET_LOC_ARRAY = 0x24; // MIPS byte offset to
														// this field
	private static final int OFFSET_TASK_COMPLETE = 0x124; // MIPS byte offset
															// to this field
	private static final int OFFSET_TASK_ARRAY = 0x128; // MIPS byte offset to
														// this field
	...
	private static int playerID = ADMINISTRATOR_ID; // Range
													// 0...(NUM_PLAYERS-1), plus
													// ADMINISTRATOR_ID
	private boolean KENVDEBUG = false;

	// Used to define (X,Y) coordinate of a location to which ScavengerHunt
	// players
	// will travel.
	...
	private class PlayerData {
		int whereAmIX = START_AND_END_LOCATION; // Read only. Memory Address:
												// Base
		int whereAmIY = START_AND_END_LOCATION; // Read only. Memory Address:
												// Base + 0x4
		...
		long finishTime;
		// int locID; // ID of the location to which ScavengerHunt players are
		// headed. Not used by player.
		boolean hasVisitedLoc[] = new boolean[NUM_LOCATIONS]; // boolean: player
																// has visited
																// each location
		boolean finis = false;
	...
	private class ScavengerHuntRunnable implements Runnable {
		...
		public ScavengerHuntRunnable() // constructor
		{
			...
			// Recommended by Pete Sanderson, 2 Nov. 2006, so that the Tool
			// window and
			// MARS window can be on the screen at the same time.
			final JDialog frame = new JDialog(Globals.getGui(), "ScavengerHunt");

			// System.out.println("ScavengerHuntRunnable.constructor:
			// starting....");
			...
			resetButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					// JOptionPane.showMessageDialog(null, "Reset needs to be
					// implemented!" );
			...
			// Snippet by Pete Sanderson, 2 Nov. 2006, to be a window-closing
			// sequence
			...
			frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); // TBD ---
																	// This
																	// should
																	// close
																	// only the
																	...
																	// entire
																	// MARS
			frame.setPreferredSize(new Dimension(GRAPHIC_WIDTH, GRAPHIC_HEIGHT)); // TBD
																					// SIZE
		...
		public void run() {
			...
			do {
				...
				// Pause to slow down the redisplay of the game. This is
				// separate from
				// the execution speed of the MIPS program, so the display may
				// lag behind
				// the state of the MIPS program.
				try {
					// System.out.println(" Hello from the ScavengerHuntRunnable
					// runner, sleeping here ...");
					// System.out.print(".");
	...
	private class ScavengerHuntDisplay extends JPanel {
		...
		public ScavengerHuntDisplay(int tw, int th) {
			// System.out.println("ScavengerHuntDisplay.constructor:
			// starting....");
			width = tw;
			height = th;
		...
		public void clear() {
			...
			// System.out.println("ScavengerHuntDisplay.clear: called to clear
			// the display");
			repaint();
		}

		/**
		 * paintComponent does not have access to the same MIPS Memory class
		 * object used by the ScavengerHunt class object. Need read-only access
		 * to a similar data structure maintained within ScavengerHunt.
		 *
		 */
		public void paintComponent(Graphics g) {
			...
			// System.out.println("ScavengerHuntDisplay.paintComponent: I'm
			// painting! n is " + n);
			...
			if (!GameOn) // Make sure game is ready before continuing
			{
				...
				g2.fillRect(0, 0, width - 1, height - 1); // Clear all previous
															// drawn information
			...
			for (int i = 0; i < NUM_LOCATIONS; i++) {
				xCoord = loc[i].X; // toolReadPlayerData(0, OFFSET_LOC_ARRAY +
									// (i*8) + 0);
				yCoord = loc[i].Y; // toolReadPlayerData(0, OFFSET_LOC_ARRAY +
									// (i*8) + 4);
				g2.setColor(Color.blue);
				g2.fillRect(xCoord, yCoord, 20, 20); // coord is upper left
														// corner of oval
				g2.setColor(Color.white);
				g2.drawString(" " + i, xCoord + 4, yCoord + 15); // coord is
																	// lower
																	// left
																	// corner of
																	// string
				...
				/*
				 * System.out.
				 * println("ScavengerHuntDisplay.paintComponent: drew loc " + i
				 * + " at (" + xCoord + ", " + yCoord + ")" );
				 */
			}

			// System.out.println("ScavengerHuntDisplay.paintComponent: special
			// exit!");
			// System.exit(0);
			...
			g2.drawLine(width - 160, 35, width - 10, 35); // line under column
															// headings
			g2.drawLine(width - 120, 35, width - 120, 35 + (NUM_PLAYERS * 15)); // vertical
																				// line
																				// for
																				// location-visited
																				// marks
			g2.drawLine(width - 50, 35, width - 50, 35 + (NUM_PLAYERS * 15)); // vertical
																				// line
																				// for
																				// location-visited
																				// marks
			for (int i = 0; i < NUM_PLAYERS; i++) {
				...
				// ystem.out.println("paintComponent loop " + i + ": Loc coord
				// is (" + xCoord + ", " + yCoord);

				// Draw player symbol and label it with player ID number.
				// Hardcoded size of location graphic and label.
				g2.drawOval(xCoord, yCoord, 20, 20); // coord is upper left
														// corner of oval
				g2.drawString(" " + i, xCoord + 4, yCoord + 15); // coord is
																	// lower
																	// left
				...
				g2.setColor(Color.black);
				g2.drawString(" " + i, width - 150, 50 + (i * 15)); // Player's
																	// ID on
																	// scoreboard
				g2.drawString(" " + pd[i].getEnergy(), width - 40, 50 + (i * 15)); // Player's
																					// energy
																					// on
																					// scoreboard

				// Display player's progress or finishing time, whichever is
				// applicable
				if (pd[i].isFinished()) {
					// Display finishing time
					// This doesn't display leading zeroes to align time
					// components (e.g. 27 millisec ought to print as 027)
					g2.drawString(
					...
					// width - 255, 50 + (i*15));
				} else // player either has not finished or is just now
						// finishing
				{
					int visCount = 0;
					for (int j = 0; j < NUM_LOCATIONS; j++) {
						if (pd[i].hasVisited(j)) // count number of locations
													// that player has visited
						{
							visCount++;
						}
					}
					if (visCount == NUM_LOCATIONS) // player has visited every
													// location -- finished!
					{
						pd[i].setFinished();
					...
					{
						// Display locations that the player has actually
						// visited
						for (int j = 0; j < NUM_LOCATIONS; j++) {
							if (pd[i].hasVisited(j)) // Player has visited this
														// location
							{
			...
			// System.out.println("paintComponent: Player " + 0 + " is at (" +
			// pd[0].getWhereAmIX() + ", " + pd[0].getWhereAmIY() + ")" );

			/*
			 * g2.setColor(Color.blue); g2.setFont(new
			 * Font(g2.getFont().getName(), g2.getFont().getStyle(), 20) ); //
			 * same font and style in larger size g2.drawOval( width/2 - 30, //
			 * TBD Hardcoded oval size height/2 - 30, 60, 60); g2.drawString(" "
			 * + n, width/2, height/2);
			 */
	...
	// A constructor for ScavengerHunt would be called immediately on MARS
	// startup, perhaps
	// before all MARS classes have been created. Do not include any action in a
	// constructor
	...
	/*
	 * This will set up the ScavengerHunt's GUI. Invoked when ScavengerHunt menu
	 * item selected.
	 */
	public void action() {
		...
		try {
			Globals.memory.addObserver(this, 0xffff8000, 0xfffffff0); // must be
																		// on
																		// word
																		// boundaries
	...
	/*
	 * This method observes MIPS memory for directives to modify ScavengerHunt
	 * activity (that is, MIPS program write to MMIO) and updates instance
	 * variables to reflect that directive. This method takes action when it
	 * "observes" MIPS memory changes -- but this method must not write to those
	 * memory locations in order to prevent an infinite cycle of events. This
	 * method observes certain locations and then may (and does) write to OTHER
	 * locations.
	 */
	public void update(Observable o, Object arg) {
		...
		// Here we are only interested in MemoryAccessNotice. For anything else,
		// just return.
		...
		// If we are only interested in MIPS memory WRITES, then just return on
		// READS.
		...
		// System.out.println("ScavengerHunt.update: observed write access by
		// player " + playerID + " on Mem[ " +
		// Binary.intToHexString(address) + " ]");

		// TBD TBD DEBUGGING SPECIAL
		/*
		 * accessCounter++; if (accessCounter > 100000) { System.out.
		 * println("\n\nScavengerHunt.update: hardcoded exit to prevent runaway"
		 * ); System.exit(0); }
		 */
		// TBD TBD DEBUGGING SPECIAL

		// Take the appropriate action, depending on data written and priority
		// of user.
		if (isWrite && playerID == ADMINISTRATOR_ID && address == ADDR_GAME_ON) {

			// ADMINISTRATOR_ID can write to any location, because it's trusted
			// software
			// System.out.println( "ScavengerHunt.update(): Administrator wrote
			// to Mem[ " +
			// Binary.intToHexString(address) + " ] == " +
			...
			// Anyone is allowed to write to the authentication location -- but
			...
			// NO ACTION HERE
		} else if (isWrite && address == ADDR_PLAYER_ID) // if the data written
															// will change the
															// PlayerID,
															// authenticate the
															// write
		{
			...
			// to students for development: Initial authentication value is
			// zero.
			// Each successive authentication value is one greater
			// than the preceding value, modulo 0xffffffff.
			authenticationValue += 1; // "server's" updated version of the
										// authenticationValue
			if (toolGetWord(ADDR_AUTHENTICATION) == authenticationValue) // Compare
																			// to
																			// "client's"
																			// version
																			// of
																			...
																			// authenticationValue
			{
				...
				// System.out.println( "ScavengerHunt.update(): New playerID of
				// " + playerID);
			} else {
		...
		else if (isWrite && address == (ADDR_BASE + (playerID * MEM_PER_PLAYER) + OFFSET_MOVE_READY) && data != 0) // Player
																													// wrote
																													// data
																													// to
																													// his/her
		...
		{
			/*
			 * System.out.println(" ******** ScavengerHunt.update: Player " +
			 * playerID + " requests move to (" + toolReadPlayerData(playerID,
			 * OFFSET_MOVE_TO_X) + ", " + toolReadPlayerData(playerID,
			 * OFFSET_MOVE_TO_Y) + ")" );
			 */

			energyLevel = toolReadPlayerData(playerID, OFFSET_ENERGY); // find
																		// if
																		// player
																		// has
																		// energy
			...
			{
				// JOptionPane.showMessageDialog(null, "Player " + playerID + "
				// can't move -- no energy.\n" +
				// "(This msg. in ScavengerHunt.update()" );
				// System.out.println("Player " + playerID + " can't move -- no
				// energy.");
				return;
			}
			...
			if (toolReadPlayerData(playerID, OFFSET_MOVE_TO_X) < 0
					...
					|| toolReadPlayerData(playerID, OFFSET_MOVE_TO_Y) > GRAPHIC_HEIGHT) // Out
																						// of
																						// bounds.
																						// Player
																						// not
			...
			if (Math.sqrt(Math
					.pow(toolReadPlayerData(playerID, OFFSET_WHERE_AM_I_X)
				...
				// Policy: Constant ENERGY_PER_MOVE for any move regardless of
				// length.
				...
				// TBD FUTURE --- need to keep track of locations that the
				// player has actually got to
				// -- be able to tell that the player has reached a certain
				// location
				...
				for (int i = 0; i < NUM_LOCATIONS; i++) {
					if (toolReadPlayerData(playerID, OFFSET_WHERE_AM_I_X) == loc[i].X
						...
						pd[playerID].setVisited(i); // Player has visited this
													// location
					}
				}

				// Write 0 to "move ready" location, signifying that the move
				// request was processed
				// Here we must write to the same location that we're now
				// reading from, and we
				// can't cause an infinite loop. Temporarily switch player ID to
				// that of the administrator,
				// and restore ID after the write. With the playerID set to
				// administrator, the event
		...
		} // end if Player wrote nonzero data to his/her assigned MoveReady
			// location

		else if (isWrite && address == (ADDR_BASE + (playerID * MEM_PER_PLAYER) + OFFSET_TASK_COMPLETE) && data != 0) // Player
																														// wrote
																														// data
																														// to
																														// his/her
																														...
																														// location
		{
			...
			// System.out.println(" ******** ScavengerHunt.update: Player " +
			// playerID + " requests more energy (task complete)" );

			// Player indicates he/she has completed a task. Check to see if
			...
			// If program flow has reached this point, the task is completed
			// correctly.
			// Award energy, reset TaskComplete, and set new task values for
			// future use.
			toolWritePlayerData(playerID, OFFSET_ENERGY, ENERGY_AWARD);
			toolWritePlayerData(playerID, OFFSET_TASK_COMPLETE, 0);
			for (int j = 0; j < SIZE_OF_TASK; j++) // Initialize the task data
													// for this player
			{
				toolWritePlayerData(playerID, OFFSET_TASK_ARRAY + (j * 4),
						(int) (randomStream.nextDouble() * Integer.MAX_VALUE)); // Set
																				// a
																				// random
																				// number
																				// for
			...
			// System.out.println("Player has completed task correctly and been
			// awarded energy");
			pd[playerID].setEnergy(ENERGY_AWARD);

		} // end if Player wrote nonzero data to his/her assigned TaskComplete
			// location
		...
		// TBD TBD TBD
		// TBD TBD TBD
		// Yet to be implemented: Enforce only one write of MoveRequest per
		// player per turn

		else if (isWrite && address >= (ADDR_BASE + (playerID * MEM_PER_PLAYER))
		...
		// Player wrote data elsewhere within his/her assigned location
		{
			...
			// System.out.println( "ScavengerHunt.update(): Player " + playerID
			// + " wrote to valid location");
			 ...
			 * Binary.intToHexString(address) + " -- not implemented!");
			 */
	...
	// Write one word to MIPS memory. This is a wrapper to isolate the
	// try..catch blocks.
	...
	// Read one word from MIPS memory. This is a wrapper to isolate the
	// try..catch blocks.
	...
	private int toolReadPlayerData(int p, int offset) {
		...
		if (KENVDEBUG) {
			...
			// (ADDR_BASE + (p * MEM_PER_PLAYER) + offset) < 0xffff8800) // Show
			// debug for player 0 and 1 only
			// {
			// System.out.println(" ScavengerHunt.toolReadPlayerData: Reading
			// MIPS Memory[" +
			// Binary.intToHexString(ADDR_BASE + (p * MEM_PER_PLAYER) + offset)
			// +
	...
	private void toolWritePlayerData(int p, int offset, int data) {
		...
		if (KENVDEBUG) {
			System.out.println("ScavengerHunt.toolWritePlayerData: called with player " + p + ", offset = "
					+ Binary.intToHexString(offset) + ", data = " + Binary.intToHexString(data));
		}
	...
	private void initializeScavengerData() {
		...
		// This is a dubious use of the tool (this Java program) to initialize
		// data values for the game.
		// The administrator portion of the MIPS program should initialize all
		// data for each
		// player -- but it's easier to work with random numbers in the Java
		// program
		// Initialize the locations and task data for each player
		randomStream = new Random(42); // TBD Use a seed for development. Remove
										// seed for randomizing.

		for (int j = 0; j < NUM_LOCATIONS - 1; j++) // The first (n-1) locations
													// are "random"
		{ // Two coordinates (x and y) for each location
			loc[j] = new Location(); // Initialize each location element
			loc[j].X = (int) (randomStream.nextDouble() * GRAPHIC_WIDTH); // X
																			// coord.
			loc[j].Y = (int) (randomStream.nextDouble() * (GRAPHIC_HEIGHT - 50)); // Y
																					// coord.
																					// (leave
																					// room
																					// for
			...
			/*
			 * System.out.
			 * println("ScavengerHunt.update(): set up a location at (" +
			 * loc[j].X + ", " + loc[j].Y + ")" );
			 */

		}
		loc[NUM_LOCATIONS - 1] = new Location(); // The last location is a
		...
		for (int i = 0; i < NUM_PLAYERS; i++) // Initialize data for each player
		{
			...
			for (int j = 0; j < NUM_LOCATIONS; j++) // Initialize the locations
													// this player goes to
			{
				toolWritePlayerData(i, OFFSET_LOC_ARRAY + (j * 8) + 0, loc[j].X); // Set
																					// the
																					// same
																					// locations
																					// for
																					// each
																					// player
				toolWritePlayerData(i, OFFSET_LOC_ARRAY + (j * 8) + 4, loc[j].Y); // Set
																					// the
																					// same
																					// locations
																					// for
