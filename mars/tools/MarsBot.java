...
public class MarsBot implements Observer, MarsTool {
	...
	private int MarsBotHeading = 0; // 0 --> North (up), 90 --> East (right),
									// etc.
	private boolean MarsBotLeaveTrack = false; // true --> leave track when
												// moving, false --> do not ...
	...
	private boolean MarsBotMoving = false; // true --> MarsBot is moving, false
											// --> MarsBot not moving

	// The begin and end points of a "track" segment are kept in neighboring
	// pairs
	// of elements of the array. arrayOfTrack[i] is the start pt,
	// arrayOfTrack[i+1] is
	// the end point of a path that should leave a track.
	private final int trackPts = 256; // TBD Hardcoded. Array contains start-end
										// points for segments in track.
		...
		public BotRunnable() // constructor
		{
			...
			clearButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					graphicArea.clear();
					MarsBotLeaveTrack = false; // true --> leave track when
												// moving, false --> do not ...
					MarsBotXPosition = 0; // X pixel position of MarsBot
					MarsBotYPosition = 0; // Y pixel position of MarsBot
					MarsBotMoving = false; // true --> MarsBot is moving, false
											// --> MarsBot not moving
			...
			frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE); // changed
																		...
																		// (was
																		// EXIT)
			frame.setSize(GRAPHIC_WIDTH + 200, GRAPHIC_HEIGHT + 100); // TBD
																		// SIZE
		...
		public void run() {
			...
			do {
				if (MarsBotMoving) {
					...
					// TBD This is an arbitrary distance for bot movement. This
					// could just
					// as easily be a random distance to simulate terrain, etc.
					// adjust bot position.
					// The "mathematical angle" is zero at east, 90 at north,
					// etc.
					// The "heading" is 0 at north, 90 at east, etc.
					// Conversion: MathAngle = [(360 - heading) + 90] mod 360
					tempAngle = ((360 - MarsBotHeading) + 90) % 360;
					MarsBotXPosition += Math.cos(Math.toRadians(tempAngle)); // Math.cos
																				// parameter
																				// unit
																				// is
																				// radians
					MarsBotYPosition += -Math.sin(Math.toRadians(tempAngle)); // Negate
																				// value
																				// because
																				// Y
																				// coord
					...
					// MarsBotHeading + ", angle is " + tempAngle);
					// System.out.println(" ------- New X,Y is (" +
					// MarsBotXPosition + "," + MarsBotYPosition + ")" );

					// Whether or not we're leaving a track, write the current
					// point to the
					// current position in the array.
					// -- If we are not leaving a track now, we will need the
					// current point to
					// start a future track, and that goes into the array.
					// -- If we are leaving a track now, the current point may
					// end the track,
					...
					// System.out.println("BotRunnable.run: bot is not
					// moving.");
				}

				// TBD Pause whether the bot is or is not moving. This gives the
				// MIPS program
				// opportunity to consider results of movement, or to make the
				// bot move.
				// ??? What is relationship of robot speed to MARS's
				// execution time for a single instruction? Does the robot speed
				// have to
				// be slow enough to allow a MARS busy loop to detect the bot
				// position
				// at a specific pixel?
				try {
	...
	private class MarsBotDisplay extends JPanel {
		...
		public void clear() {
			...
			// System.out.println("MarsBotDisplay.clear: called to clear the
			// display");
			repaint();
		}
		...
		public void paintComponent(Graphics g) {
			long tempN;
			// System.out.println("MarsBotDisplay.paintComponent: I'm painting!
			// n is " + n);
			...
			/*
			 * if (clearTheDisplay) { g2.setColor(Color.lightGray);
			 * g2.fillRect(0, 0, width - 1, height - 1); // Clear all previous
			 * drawn information clearTheDisplay = false; }
			 */

			// Draw the track left behind, for each segment of the path
			g2.setColor(Color.blue);
			for (int i = 1; i <= trackIndex; i += 2) // Index grows by two
														// (begin-end pair)
			{
				// System.out.print(".");
				try {
			...
			g2.fillRect((int) MarsBotXPosition, (int) MarsBotYPosition, 20, 20); // Draw
																					// bot
																					// at
																					// its
																					// current
	...
	/*
	 * This method observes MIPS program directives to modify Bot activity (that
	 * is, MIPS program write to MMIO) and updates instance variables to reflect
	 * that directive.
	 */
		...
		if (arg instanceof MemoryAccessNotice) {
			...
			if (address < 0 && notice.getAccessType() == AccessNotice.WRITE) {
				...
				if (address == ADDR_HEADING) {
					...
					// If we HAD NOT been leaving a track, but we should NOW
					// leave
					...
					// If we HAD NOT been leaving a track, and get another
					// directive
					...
					// System.out.println("MarsBotDisplay.paintComponent:
					// putting point in track array at " + trackIndex);
					...
					// Ignore these memory writes, because the writes originated
					// within
					// this tool. This tool is being notified of the writes in
					// the usual
					// manner, but the writes are already known to this tool.
					// NO ACTION
				} else {
					// message = "MarsBot.update: HEY!!! unknown address of " +
					// Integer.toString(address) + ", value: ";
					// System.out.println(message + notice.getValue() );
				}
