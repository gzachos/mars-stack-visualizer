...
public class HardcopyWriter extends Writer {
	...
	public HardcopyWriter(Frame frame, String jobname, int fontsize, double leftmargin, double rightmargin,
		...
		/*******************************************************
		 * SANDERSON OVERRIDE 8-17-2004: I didn't like the results produced by
		 * the code below, so am commenting it out and just setting pagedpi to
		 * 72. This assures, among other things, that the client asking for 10
		 * point font will really get 10 point font!
		 * 
		 * 
		 * pagesize = job.getPageDimension( ); // query the page size pagedpi =
		 * job.getPageResolution( ); // query the page resolution // Bug
		 * Workaround: // On Windows, getPageDimension( ) and getPageResolution
		 * don't work, so // we've got to fake them. if
		 * (System.getProperty("os.name").regionMatches(true,0,"windows",0,7)){
		 * // Use screen dpi, which is what the PrintJob tries to emulate
		 * pagedpi = toolkit.getScreenResolution( ); // Assume a 8.5" x 11" page
		 * size. A4 paper users must change this. pagesize = new
		 * Dimension((int)(8.5 * pagedpi), 11*pagedpi); // We also have to
		 * adjust the fontsize. It is specified in points, // (1 point = 1/72 of
		 * an inch) but Windows measures it in pixels. fontsize = fontsize *
		 * pagedpi / 72; }
		 ***********************************/
