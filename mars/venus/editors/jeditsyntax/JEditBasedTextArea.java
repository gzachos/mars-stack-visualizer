...
public class JEditBasedTextArea extends JEditTextArea implements MARSTextEditingArea, CaretListener {
	...
	public int doFindText(String find, boolean caseSensitive) {
		...
		if (nextPosn >= 0) {
			...
			// Need to repeat start due to quirk in JEditTextArea implementation
			// of setSelectionStart.
		...
		while (nextPosn >= 0) {
			...
			if (nextPosn >= 0) {
				...
				sourceCode.setSelectionEnd(nextPosn + find.length()); // select
																		// found
																		// text
				// Need to repeat start due to quirk in JEditTextArea
				// implementation of setSelectionStart.
				sourceCode.setSelectionStart(nextPosn);
