...
public class SyntaxUtilities {
	...
	public static SyntaxStyle[] getDefaultSyntaxStyles() {
		...
		// All need to be assigned even if not used by language (no gaps in
		// array)
		...
		styles[Token.KEYWORD3] = new SyntaxStyle(Color.red, false, false);// (new
																			// Color(0x009600),false,false);
		styles[Token.LITERAL1] = new SyntaxStyle(new Color(0x00CC33), false, false);// (new
																					// Color(0x650099),false,false);
		styles[Token.LITERAL2] = new SyntaxStyle(new Color(0x00CC33), false, false);// (new
																					// Color(0x650099),false,true);
		styles[Token.LABEL] = new SyntaxStyle(Color.black, true, false);// (new
	 ...
	 * <code>setStyles()</code> method of <code>SyntaxDocument</code> to use the
	 * current syntax styles. If changes have been made via MARS Settings menu,
	 * the current settings will not be the same as the default settings.
	 */
		...
		for (;;) {
			...
			if (id == Token.KEYWORD1) {
				...
				if (!popupShowing) {// System.out.println("creating popup");
					...
					// popup = popupFactory.getPopup(paintArea, tip,
					// screenLocation.x + x, screenLocation.y + y);
					// popupShowing = true;
					// popup.show();
				...
				// new MouseEvent((Component)expander, MouseEvent.MOUSE_MOVED,
				// new java.util.Date().getTime(), 0, x, y, 0, false));
