...
public class JEditTextArea extends JComponent {
	...
	// Number of text lines moved for each click of the vertical scrollbar
	// buttons.
	...
	public JEditTextArea(TextAreaDefaults defaults, JComponent lineNumbers) {
		...
		JScrollPane lineNumberScroller = new JScrollPane(lineNumbers, ScrollPaneConstants.VERTICAL_SCROLLBAR_NEVER,
	...
	public void updateScrollBars() {
		if (vertical != null && visibleLines != 0) {
			...
			// Editing area scrollbar has custom model that increments by number
			// of text lines instead of
			// number of pixels. The line number display uses a standard (but
			// invisible) scrollbar based
			// on pixels, so I need to adjust accordingly to keep it in synch
			// with the editing area scrollbar.
			// DPS 4-May-2010
	...
	class DocumentHandler implements DocumentListener {
		public void insertUpdate(DocumentEvent evt) {
			...
			// If event fired because of undo or redo, select inserted text. DPS
			// 3-May-2010
			if (unredoing) {
		...
		public void removeUpdate(DocumentEvent evt) {
			...
			// If event fired because of undo or redo, move caret to position of
			// removal. DPS 3-May-2010
			if (unredoing) {
	 ...
	 * Keyword match must be exact. DPS 24-May-2010
	 * 
	 * @param x
	 *            x-coordinate of current position
	 * @param y
	 *            y-coordinate of current position
	 * @return String containing appropriate tool tip text. Possibly
	 *         HTML-encoded.
	 */
	 ...
	 * getLeadingWhiteSpace(). Is used by InputHandler when processing key press
	 * for Enter key. DPS 31-Dec-2010
	 * 
	...
	/**
	 * Makes a copy of leading white space (tab or space) from the current line
	 * and returns it. DPS 31-Dec-2010
	 * 
	 * @return String containing leading white space of current line. Empty
	 *         string if none.
	 */
	...
	private ArrayList getSyntaxSensitiveHelpAtLineOffset(int line, int offset, boolean exact) {
		...
		if (tokenMarker != null) {
			...
			this.getLineText(line, lineSegment); // fill segment with info from
													// this line
	...
	////////////////////////////////////////////////////////////////////////////////////
	// Compose and display syntax-sensitive help. Typically invoked upon typing
	//////////////////////////////////////////////////////////////////////////////////// a
	//////////////////////////////////////////////////////////////////////////////////// key.
	...
	private void applySyntaxSensitiveHelp() {
		...
		if (helpItems != null) {
			...
			for (int i = 0; i < helpItems.size(); i++) {
				...
				if (item.getExact()) {
					// The instruction name is completed so the role of the
					// popup changes
					// to that of floating help to assist in operand
					// specification.
					menuItem.setSelected(false);
					// Want menu item to be disabled but that causes rendered
					// text to be hard to see.
					// Spent a couple hours on workaround with no success. The
					// UI uses
					// UIManager.get("MenuItem.disabledForeground") property to
					// determine rendering
					// color but this is done each time the text is rendered
					// (paintText). There is
					// no setter for the menu item itself. The UIManager
					// property is used for all
					// menus not just the editor's popup help menu, so you can't
					// just set the disabled
					// foreground color to, say, black and leave it. Tried
					// several techniques without
					// success. The only solution I found was a hack: writing a
					// BasicMenuItem UI
					// subclass that consists of hacked override of its
					// paintText() method. But even
					// this required use of "SwingUtilities2" class which has
					// been deprecated for years
					// So in the end I decided just to leave the menu item
					// enabled. It will highlight
					// but does nothing if selected. DPS 11-July-2014

					// menuItem.setEnabled(false);
				} else {
					// Typing of instruction/directive name is still in
					// progress; the action listener
			...
			// current cursor position is low enough, the menu will bottom out
			// at the
			// bottom of the screen and extend above the current line. DPS
			// 23-Dec-2010
	...
	private class PopupHelpActionListener implements ActionListener {
		...
		// Completion action will insert either a tab or space character
		// following the
	...
	////////////////////////////////////////////////////////////////////////////////////
	// Called after processing a Key Pressed event. Will make popup menu
	//////////////////////////////////////////////////////////////////////////////////// disappear
	//////////////////////////////////////////////////////////////////////////////////// if
	...
	////////////////////////////////////////////////////////////////////////////////////
	// Called before processing Key Pressed event. If popup menu is visible,
	//////////////////////////////////////////////////////////////////////////////////// will
	//////////////////////////////////////////////////////////////////////////////////// process
	// tab and enter keys to select from the menu, and arrow keys to traverse
	//////////////////////////////////////////////////////////////////////////////////// the
	//////////////////////////////////////////////////////////////////////////////////// menu.
