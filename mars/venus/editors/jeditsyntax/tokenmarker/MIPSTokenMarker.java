...
public class MIPSTokenMarker extends TokenMarker {
	...
	public byte markTokensImpl(byte token, Segment line, int lineIndex) {
		...
		loop: for (int i = offset; i < length; i++) {
			...
			case Token.NULL:
				...
				case ':':
					/*
					 * Original code for ':' case, replaced 3 Aug 2010. Details
					 * below. if(lastKeyword == offset) { // Commenting out this
					 * IF statement permits recognition of keywords // used as
					 * labels when terminated with ":". The most common example
					 * // is "b:" (where b is mnemonic for branch instruction).
					 * DPS 6-July-2010. // // if(doKeyword(line,i,c)) // break;
					 * backslash = false; addToken(i1 - lastOffset,Token.LABEL);
					 * lastOffset = lastKeyword = i1; } else
					 * if(doKeyword(line,i,c)) break; break;
					 */
					// Replacement code 3 Aug 2010. Will recognize label
					// definitions when:
					// (1) label is same as instruction name, (2) label begins
					// after column 1,
					// (3) there are spaces between label name and colon, (4)
					// label is valid
					// MIPS identifier (otherwise would catch, say, 0 (zero) in
					// .word 0:10)
					backslash = false;
					// String lab = new String(array, lastOffset,
					// i1-lastOffset-1).trim();
	...
	/**
	 * Construct and return any appropriate help information for the given
	 * token.
	 * 
	 * @param token
	 *            the pertinent Token object
	 * @param tokenText
	...
	/**
	 * Construct and return any appropriate help information for prefix match
	 * based on current line's token list.
	 * 
	 * @param line
	 *            String containing current line
	 * @param tokenList
	 *            first Token on current line (head of linked list)
	 * @param token
	 *            the pertinent Token object
	 * @param tokenText
	 *            the source String that matched to the token in previous
	 *            parameter
	 * @return ArrayList of PopupHelpItem objects, one per match.
	 */
	...
	public ArrayList getTokenPrefixMatchHelp(String line, Token tokenList, Token token, String tokenText) {
		...
		// Let's see if the line already contains an instruction or directive.
		// If so, we need its token
		// text as well so we can do the match. Also need to distinguish the
		// case where current
		...
		// CASE: Current token is valid KEYWORD1 (MIPS instruction). If this
		// line contains a previous KEYWORD1 or KEYWORD2
		// token, then we ignore this one and do exact match on the first one.
		// If it does not, there may be longer
		// instructions for which this is a prefix, so do a prefix match on
		// current token.
		if (token != null && token.id == Token.KEYWORD1) {
		...
		// CASE: Current token is valid KEYWORD2 (MIPS directive). If this line
		// contains a previous KEYWORD1 or KEYWORD2
		// token, then we ignore this one and do exact match on the first one.
		// If it does not, there may be longer
		// directives for which this is a prefix, so do a prefix match on
		// current token.
		if (token != null && token.id == Token.KEYWORD2) {
			if (moreThanOneKeyword) {
				return (keywordType == Token.KEYWORD1) ? getTextFromInstructionMatch(keywordTokenText, true)
		...
		// something other
		// than KEYWORD1 or KEYWORD2. Generate text based on exact match of that
		// token.
		if (keywordTokenText != null) {
		...
		// CASE: Current token is NULL, which can be any number of things. Think
		// of it as being either white space
		// or an in-progress token possibly preceded by white space. We'll do a
		// trim on the token. Now there
		// are two subcases to consider:
		// SUBCASE: The line does not contain any KEYWORD1 or KEYWORD2 tokens
		// but nothing remains after trimming the
		// current token's text. This means it consists only of white space and
		// there is nothing more to do
		// but return.
		// SUBCASE: The line does not contain any KEYWORD1 or KEYWORD2 tokens.
		// This means we do a prefix match of
		// of the current token to either instruction or directive names. Easy
		...
		if (token != null && token.id == Token.NULL) {
			...
			// Subcase: no KEYWORD1 or KEYWORD2 but current token contains
			// nothing but white space. We're done.
			if (keywordTokenText == null && trimmedTokenText.length() == 0) {
				return null;
			}

			// Subcase: no KEYWORD1 or KEYWORD2. Generate text based on prefix
			// match of trimmed current token.
			if (keywordTokenText == null && trimmedTokenText.length() > 0) {
	...
	///////////////////////////////////////////////////////////////////////////
	// Return ArrayList of PopupHelpItem for match of directives. If second
	/////////////////////////////////////////////////////////////////////////// argument
	...
	// Return text for match of instruction mnemonic. If second argument true,
	// will
	// do exact match. If false, will do prefix match. Text is returned as
	// ArrayList
	...
	/**
	 * Get KeywordMap containing all MIPS key words. This includes all
	 * instruction mnemonics, assembler directives, and register names.
	 *
	 * @return KeywordMap where key is the keyword and associated value is the
	 *         token type (e.g. Token.KEYWORD1).
	 */
	...
	public static KeywordMap getKeywords() {
		if (cKeywords == null) {
			...
			for (int i = 0; i < registerFile.length; i++) {
				...
				cKeywords.add("$" + i, Token.KEYWORD3); // also recognize $0,
														// $1, $2, etc
			}
			...
			// Note: Coprocessor 0 registers referenced only by number: $8, $12,
			// $13, $14. These are already in the map
