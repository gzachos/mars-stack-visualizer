 ...
 * the timer is active, it will be stopped.
 * 
 * NOTE: The normal button behavior is that the action event is fired after the
 * button is released. It may be important to konw then that this is still the
 * case. So in effect, listeners will get 1 more event then what the internal
 * timer fires. It's not a "bug", per se, just something to be aware of. There
 * seems to be no way to suppress the final event from firing anyway, except to
 * process all ActionListeners internally. But realistically, it probably
 * doesn't matter.
 */
public class RepeatButton extends JButton implements ActionListener, MouseListener {
	...
	/**
	 * Flag to indicate that the button should fire events when held. If false,
	 * the button is effectively a plain old JButton, but there may be times
	...
	/**
	 * The initial delay for this button. Hold-down time before first timer
	 * firing. In milliseconds.
	 */
	...
	/**
	 * Holder of the modifiers used when the mouse pressed the button. This is
	 * used for subsequently fired action events. This may change after mouse
	 * pressed if the user moves the mouse out, releases a key and then moves
	 * the mouse back in.
	 */
	...
	/**
	 * Checks if the button should fire events when held. If false, the button
	 * is effectively a plain old JButton, but there may be times when this
	...
	/**
	 * Sets the enabled state of this button. Overridden to stop the timer if
	 * it's running.
	 * 
	 * @param en
